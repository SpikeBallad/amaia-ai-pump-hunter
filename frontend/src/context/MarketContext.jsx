...
function mergeTopRows(previousRows, topRows) {
  ...
  topRows.forEach((row) => {
    ...
    mergedById.set(rowId, {
      ...
      score: row.score,
      estado: row.estado,
      setupDirection: row.setup_direction ?? mergedById.get(rowId)?.setupDirection ?? 'pump',
      ...
      isBreakingOut: (row.status_label ?? '').includes('Starting'),
      summary: row.setup,
  ...
  const triggerAlert = useCallback(
    async (row, source = 'stream') => {
      const alertItem = {
        ...
        score: row.score,
        estado: row.estado,
        setupDirection: row.setupDirection ?? 'pump',
